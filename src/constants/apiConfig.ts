...
const USE_DEPLOYED_BACKEND: boolean = true; // ✅ Using deployed backend
const DEPLOYED_API_URL: string = 'https://backend-of-nemo.vercel.app';

// OPTION 2: Use local backend (for development)
// Only works when testing on emulator/simulator or same network
const COMPUTER_IP: string = '192.168.1.6'; // Your computer's IP address
const API_PORT: number = 5000;
const LOCAL_API_URL: string = `http://${COMPUTER_IP}:${API_PORT}`;

// Export the active API base URL
export const API_BASE_URL: string = USE_DEPLOYED_BACKEND ? DEPLOYED_API_URL : LOCAL_API_URL;
export const IMGBB_API_KEY: string = 'f29449e712111ed5e49dbc6e43c00d09';
